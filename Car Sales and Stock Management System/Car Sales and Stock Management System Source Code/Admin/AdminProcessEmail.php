...
function SendEmail($r){
	...
	$sql = "SELECT * FROM admin WHERE Email = '$r'";		//Check the email
	echo $sql;
	...
	if(mysqli_num_rows($result) == 1){
		...
		try {
			...
			if ($mail->send()) {
				...
				$reset_password = "UPDATE admin SET Password = '$hash_password' WHERE ID = '$ID'";
				if(mysqli_query($conn,$reset_password)){
					$_SESSION["AdminisSent"] = true;
